/**
 * C Program for the Max_Heap data structure
 * Created By: cRYP70N
 * Date: 30/09/2020
 */
...
// This is a utility function to create our heap
...
// the downheapify function to correct any violations
...
// Up Heapify to correct any violations in our heap
...
// To push a new item to our heap
...
void pop(Heap *heap)
{
	...
	*(heap->p) = temp;
	...
	// Correct any violation of our heap
	down_heapify(heap, 0);
	if (4 * heap->count <= heap->size)
	{
		heap->size /= 2;
		(heap->p) = realloc((heap->p), (heap->size) * sizeof(int));
	}
}

// Return the value of our very top element in the Max Heap
int top(Heap *heap)
{
	if (heap->count != 0)
		return *(heap->p);
...
// How many elements do we still have in our heap
int size(Heap *heap)
{
	return heap->count;
}